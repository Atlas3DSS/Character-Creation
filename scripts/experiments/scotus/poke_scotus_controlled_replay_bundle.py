#!/usr/bin/env python3
"""Generated-token bundle poke for controlled SCOTUS replay directions.

This runner tests the "shape, not one knob" hypothesis on controlled replay
features. It builds paired target-reference deltas across several layers, then
injects the resulting bundle during generation. The `decode` position skips the
prompt prefill and only edits generated-token forward passes.
"""
...
DEFAULT_REPLAY_RUN = PROJECT_ROOT / "sweep_v4" / "scotus_article3_controlled_replay_v2_20260501_172957"
    ...
    *,
    ...
    for idx, row in enumerate(meta):
        if str(row.get("split")) not in fit_splits:
            continue
        pair_id = str(row.get(pair_field, ""))
        if not pair_id:
            continue
        label = str(row.get(label_field))
        grouped.setdefault(pair_id, {})[label] = idx

    pairs: list[tuple[str, int, int]] = []
    for pair_id, labels in sorted(grouped.items()):
        if target_label in labels and reference_label in labels:
            pairs.append((pair_id, labels[reference_label], labels[target_label]))
    if not pairs:
        raise RuntimeError(
            "No paired rows found for "
            f"{reference_label!r}->{target_label!r} using {pair_field=} {label_field=} "
            f"and splits {sorted(fit_splits)}"
        )
    return pairs
    ...
    *,
    ...
    try:
        for layer in layers:
            ...
            values = features[key].astype(np.float32, copy=False)
            deltas = np.stack([values[target_idx] - values[reference_idx] for _, reference_idx, target_idx in pairs])
            mean_delta = deltas.mean(axis=0)
            if mode == "mean":
                direction = unit(mean_delta)
                raw_norm = float(np.linalg.norm(mean_delta))
            elif mode == "pc1":
                direction = unit(oriented_pc1(deltas, mean_delta))
                raw_norm = 1.0
            else:
            ...
            direction_meta.append(
                {
                    "source": "controlled_replay_bundle",
                    "name": f"controlled_{mode}_{region}_L{layer:02d}",
                    "layer": layer,
                    "region": region,
                    "group_field": label_field,
                    "group_value": target_label,
                    "target_justice": target_label,
                    "reference_justice": reference_label,
                    "n_target": len(pairs),
...
def install_bundle_hooks(
    ...
    *,
    ...
    if position not in {"last", "all", "decode"}:
        raise ValueError(f"Unsupported bundle position: {position}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Poke controlled SCOTUS replay layer bundles.")
    ...
    parser.add_argument("--random-controls", type=int, default=6)
    ...
    parser.add_argument("--report-max-output-rows", type=int, default=120)
...
def main() -> None:
    ...
    enforce_complete_answer_budget(
        ...
        purpose="controlled replay bundle poke",
    )
    started = now_iso()
    out_dir = args.output_root / f"scotus_controlled_bundle_poke_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(
        f"Built bundle {candidate_name} layers={layers} mode={args.mode} position={args.position}",
        flush=True,
    )
