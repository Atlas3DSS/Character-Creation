...
DEFAULT_TAG = "book_character_prefill_activation_probe_v1"
...
def extract_features(rows: list[dict[str, Any]], model_path: str, device_map: str, out_dir: Path) -> dict[str, Any]:
            ...
            all_meta.append(
                {
                    ...
                    "label": int(row["label"]),
                    "pair_quality": int(row.get("pair_quality", 0) or 0),
...
def main() -> None:
    ...
    extracted = extract_features(rows, args.model_path, args.device_map, out_dir)
