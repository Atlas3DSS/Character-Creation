...
STATUS_MD="$OUT_ABS/progress_status.md"
...
EOF
  ...
  cat >"$STATUS_MD" <<EOF
# Personality Control Reasoning V3

- Timestamp: $ts
- Output: $OUT_ABS
- Completed: $completed
- Expected: $expected
- Remaining: $(( expected > completed ? expected - completed : 0 ))
- 3090 shard alive: $shard0_alive
- 4090 shard alive: $shard1_alive
EOF
  ...
  if [[ "$expected" -gt 0 && "$completed" -ge "$expected" && "$shard0_alive" -eq 0 && "$shard1_alive" -eq 0 ]]; then
    touch "$DONE_FILE"
    echo "[$ts] complete"
    tmux has-session -t "$S3090" 2>/dev/null && tmux kill-session -t "$S3090"
    tmux has-session -t "$S4090" 2>/dev/null && tmux kill-session -t "$S4090"
    break
  fi
