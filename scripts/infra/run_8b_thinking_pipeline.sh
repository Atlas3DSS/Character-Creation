...
#
# Usage: bash scripts/infra/run_8b_thinking_pipeline.sh
# Logs: ./pipeline_gpu_a.log, ./pipeline_gpu_b.log

set -e

VENV="$HOME/dev_genius/venv"
WORKDIR="$HOME/dev_genius/experiments/Character Creation"
...
# GPU A (4090): L9 + L15
CUDA_VISIBLE_DEVICES=0 nohup python3 -u scripts/sae/sae_collect_8b_thinking.py \
    --layers 9 15 \
    --max-tokens 500000 \
    --max-gen-tokens 512 \
    --n-reps 4 \
...
# GPU B (3090): L22 + L29
CUDA_VISIBLE_DEVICES=1 nohup python3 -u scripts/sae/sae_collect_8b_thinking.py \
    --layers 22 29 \
    --max-tokens 500000 \
    --max-gen-tokens 512 \
    --n-reps 4 \
...
CUDA_VISIBLE_DEVICES=0 nohup python3 -u scripts/experiments/personality/personality_sweep_collector.py \
    --split odd \
...
CUDA_VISIBLE_DEVICES=1 nohup python3 -u scripts/experiments/personality/personality_sweep_collector.py \
    --split even \
